...
constexpr std::string_view BackgroundImagePath =
    "graphics/gameintrobackground.png";
...
const std::string RawIntroText =
    "Long ago, a contract was made and the Realm of Enemata was sealed away."
    "             \n"
...
std::string formatStoryText(const std::string &input,
                            const std::size_t maxLineLength = 36) {
  std::string result;
  ...
  while (std::getline(stream, line)) {
    ...
    // Separate trailing whitespace pauses from printable line content
    std::size_t rPos = line.size();
    while (rPos > 0 && line[rPos - 1] == ' ') {
      --rPos;
    }
    const std::string trimmedContent = line.substr(0, rPos);
    const std::string trailingSpaces = line.substr(rPos);
    ...
    while (i < trimmedContent.size()) {
      if (trimmedContent[i] == ' ') {
        std::size_t start = i;
        while (i < trimmedContent.size() && trimmedContent[i] == ' ') {
          ++i;
        }
        if (!currentLine.empty()) {
          currentLine += trimmedContent.substr(start, i - start);
        }
      } else {
        std::size_t start = i;
        while (i < trimmedContent.size() && trimmedContent[i] != ' ') {
        ...
        const std::string word = trimmedContent.substr(start, i - start);
        if (currentLine.empty()) {
          currentLine = word;
        } else if (currentLine.size() + word.size() <= maxLineLength) {
          currentLine += word;
        } else {
    ...
    // Re-attach trailing spaces to the last line of the paragraph for the typewriter pause
    wrappedLines.back() += trailingSpaces;
...
GameIntroState::GameIntroState(StateManager *l_statemanager,
                               SharedContext *l_context)
    : State(l_statemanager, l_context),
      m_formattedStory(formatStoryText(RawIntroText)), m_passedTime(0.0f),
      m_revealSpeed(12.0f), m_stormcloudPos(InitialStormcloudPos),
      m_darkness(1.0f), m_fadeSpeed(0.02f) {}

void GameIntroState::onCreate() {
  // 1. Background image
  const auto bgPath = utils::getResourcePath(BackgroundImagePath);
  if (m_context->m_resourceManager->loadTexture(
          std::string(BackgroundTextureId), bgPath)) {
    m_backgroundSprite.emplace(m_context->m_resourceManager->getTexture(
        std::string(BackgroundTextureId)));
    m_hasLoadedBackground = true;
  } else {
    std::println(stderr,
                 "GameIntroState [WARNING]: Failed to load background '{}'.",
                 bgPath.string());
  }
  ...
  if (m_context->m_resourceManager->loadTexture(
          std::string(StormCloudTextureId), cloudPath)) {
    m_stormcloudSprite.emplace(m_context->m_resourceManager->getTexture(
        std::string(StormCloudTextureId)));
    ...
    std::println(stderr,
                 "GameIntroState [WARNING]: Failed to load storm cloud '{}'.",
                 cloudPath.string());
  }
  ...
  if (!m_context->m_resourceManager->loadFont(std::string(FontId),
    ...
    if (!m_context->m_resourceManager->loadFont(std::string(FontId),
                                                fallbackFontPath)) {
      std::println(stderr, "GameIntroState [ERROR]: Failed to load font '{}'.",
...
//  m_storyText->setOutlineThickness(1.0f);
  ...
  // 5. Skip prompt setup
  m_skipPromptText.emplace(font, "press space or enter to continue",
                           SkipPromptCharacterSize);
...
//  m_skipPromptText->setOutlineThickness(1.0f);
  ...
  if (m_context && m_context->m_audioManager) {
    ...
    if (!m_context->m_audioManager->hasTrack(std::string(IntroMusicTrackId))) {
      m_context->m_audioManager->loadTrack(std::string(IntroMusicTrackId),
                                           musicPath);
    }
  }
}
...
void GameIntroState::onDestroy() {
  ...
  if (m_hasLoadedBackground) {
    m_context->m_resourceManager->releaseTexture(
        std::string(BackgroundTextureId));
...
void GameIntroState::update(const float l_dt) {
  ...
  const auto charsToShow =
      static_cast<std::size_t>(m_passedTime * m_revealSpeed);
  if (m_storyText) {
  ...
  if (m_passedTime > 42.0f && m_skipPromptText) {
    ...
    const auto flickerAlpha =
        static_cast<std::uint8_t>(((sinVal + 1.0f) / 2.0f) * 255.0f);
